...
{
    static class SaveToExcelStorekeeper
    {
        public static void CreateDoc(ListCarInfoStorekeeper info)
        {
                ...
                .Create(info.FileName, SpreadsheetDocumentType.Workbook))
            {
               ...
               spreadsheetDocument.WorkbookPart.AddNewPart<SharedStringTablePart>();
                ...
                foreach (var carwork in info.CarWork)
                {
                    InsertCellInWorksheet(new ExcelCellParameters
                    {
                        Worksheet = worksheetPart.Worksheet,
                        ...
                        ColumnName = "A",
                        RowIndex = rowIndex,
                        Text = carwork.Value.CarName,
                        StyleIndex = 0U
                    });
                    rowIndex++;
                    foreach (var work in carwork.Value.Works)
                    {
                        InsertCellInWorksheet(new ExcelCellParameters
                        {
                            Worksheet = worksheetPart.Worksheet,
