...
{
    static class SaveToExcelWorker
    {
        public static void CreateDoc(ListSparePartsInfoWorker info)
        {
                ...
                .Create(info.FileName, SpreadsheetDocumentType.Workbook))
            {
               ...
               spreadsheetDocument.WorkbookPart.AddNewPart<SharedStringTablePart>();
                ...
                foreach (var tmsp in info.TechnicalMaintenanceSpareParts)
                {
                    InsertCellInWorksheet(new ExcelCellParameters
                    {
                        Worksheet = worksheetPart.Worksheet,
                        ...
                        ColumnName = "A",
                        RowIndex = rowIndex,
                        Text = tmsp.TechnicalMaintenanceName,
                        StyleIndex = 0U
                    });
                    rowIndex++;
                    foreach (var tm in tmsp.SpareParts)
                    {
                        InsertCellInWorksheet(new ExcelCellParameters
                        {
                            Worksheet = worksheetPart.Worksheet,
